...
pub(crate) fn resolve_study_root(app: &AppHandle, project_id: &str, study_id: &str) -> Result<PathBuf, String> {
  ...
  if !study.folder_path.trim().is_empty() {
    ...
    Ok(PathBuf::from(project.root_path.clone()).join("studies").join(study_id))
  }
}
...
#[tauri::command]
pub fn list_build_assets(app: AppHandle, project_id: String, study_id: String) -> Result<Vec<AssetRef>, String> {
...
#[tauri::command]
pub fn list_prereg_assets(app: AppHandle, project_id: String, study_id: String) -> Result<Vec<AssetRef>, String> {
  let root = resolve_study_root(&app, &project_id, &study_id)?;
  ...
  Ok(out
    ...
    .filter(|a| {
      let p = a.path.to_lowercase();
      p.ends_with(".docx") || p.ends_with(".md") || p.ends_with(".markdown") || p.ends_with(".json") || p.ends_with(".txt")
    })
    .collect())
}
