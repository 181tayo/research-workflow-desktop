...
pub fn fill_from_text(spec: &mut PreregSpec, text: &str) {
  if spec.variables.dv.is_empty() {
    spec.variables.dv = extract_list_after_markers(text, &["dv", "dependent variable", "dependent variables"]);
  }
  if spec.variables.iv.is_empty() {
    spec.variables.iv = extract_list_after_markers(text, &["iv", "independent variable", "independent variables"]);
  }
  ...
  if !models.is_empty() {
    ...
    if spec.variables.dv.is_empty() {
      let mut dvs = spec.main_analyses.iter().map(|m| m.dv.clone()).collect::<Vec<String>>();
      dvs.sort();
      dvs.dedup();
    ...
    spec.main_analyses.push(AnalysisModelSpec {
      ...
      formula: Some(format!("{} ~ {}", spec.variables.dv[0], spec.variables.iv.join(" + "))),
    });
  }
...
pub fn extract_list_after_markers(text: &str, markers: &[&str]) -> Vec<String> {
  let mut out = Vec::new();
  let heading_re = Regex::new(r"(?im)^\s*(\d+\)|#+\s+|[A-Za-z][A-Za-z \t]{0,60}:)\s*$").expect("regex");
  for marker in markers {
    ...
    let marker_heading = Regex::new(&format!(r"(?im)^\s*{}\s*:?\s*$", regex::escape(marker))).expect("regex");
  ...
  if out.is_empty() {
    for (idx, cap) in regress_re.captures_iter(text).enumerate() {
      ...
      out.push(AnalysisModelSpec {
        ...
        formula: Some(format!("{} ~ {}", dv, iv.iter().chain(controls.iter()).cloned().collect::<Vec<String>>().join(" + "))),
      });
    }
  }
  out
...
pub fn extract_scales(text: &str) -> Vec<DerivedScale> {
  ...
  for cap in text_re.captures_iter(text) {
    ...
    if name.is_empty() || out.iter().any(|s: &DerivedScale| s.name == format!("{}_scale", name)) {
      continue;
    }
    out.push(DerivedScale {
      name: format!("{}_scale", name),
...
pub fn extract_robustness(text: &str) -> Vec<String> {
  ...
  if (lc.contains("without any control variables") && lc.contains("controlling for participant demographics"))
...
fn extract_concepts_after_markers(text: &str, markers: &[&str]) -> Vec<String> {
  ...
  for marker in markers {
    ...
    for cap in inline.captures_iter(text) {
      if let Some(m) = cap.get(1) {
        for item in split_candidates(m.as_str()) {
          ...
          if !normalized.is_empty() && !out.iter().any(|v| v.eq_ignore_ascii_case(&normalized)) {
            out.push(normalized);
          }
        }
      }
    ...
    let marker_heading = Regex::new(&format!(
      r"(?im)^\s*{}\s*:?\s*$",
      regex::escape(marker)
    ))
    .expect("regex");
...
fn plausible_variable_token(token: &str) -> bool {
  ...
  if Regex::new(r"^[a-z]+[A-Z][A-Za-z0-9]*$").expect("regex").is_match(value) {
    return true;
  }
...
"#;
    ...
    assert!(spec.main_analyses.iter().all(|m| m.iv.contains(&"income_condition".to_string())));
    assert!(spec.main_analyses.iter().all(|m| m.iv.contains(&"information_condition".to_string())));
    assert!(spec.robustness_checks.contains(&"with_without_controls".to_string()));
    assert!(!spec.warnings.iter().any(|w| w == "NO_MAIN_ANALYSIS_EXTRACTED"));
  }
