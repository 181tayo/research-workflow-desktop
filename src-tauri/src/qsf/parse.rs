...
pub fn parse_qsf_json_with_tokens(raw: &str, candidate_tokens: &[String]) -> Result<QsfSurveySpec, String> {
...
fn parse_question(payload: &Value, token_filters: &[String]) -> Option<QsfQuestion> {
  ...
  if !token_filters.is_empty() {
    ...
    let keep = token_filters.iter().any(|token| token_match_score(token, &n_tag) >= 0.55 || token_match_score(token, &n_text) >= 0.55);
    if !keep {
      return None;
    }
  }
...
mod tests {
  ...
  fn parses_sq_and_fl_only_with_embedded_data_defaults() {
    ...
    assert!(spec.embedded_data_fields.iter().any(|f| f.name == "condition" && f.default_value.as_deref() == Some("treat")));
  ...
  fn targeted_mode_keeps_condition_label_and_info_aliases() {
    ...
    let tokens = vec!["income_condition".to_string(), "information_condition".to_string()];
    let spec = parse_qsf_json_with_tokens(raw, &tokens).expect("parse qsf targeted");
    let tags = spec.questions.iter().map(|q| q.export_tag.clone()).collect::<Vec<String>>();
