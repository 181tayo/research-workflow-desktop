...
fn resolved_or_todo(var: &str, mappings: &[MappingResult], unresolved: &mut Vec<String>) -> String {
    if let Some(m) = mappings
        .iter()
        .find(|m| m.prereg_var.eq_ignore_ascii_case(var))
    {
...
fn build_counterbalance_derived_variables(
    ...
    for m in mappings {
        ...
        if expected
            .iter()
            .any(|col| col.eq_ignore_ascii_case(resolved))
        {
            continue;
        }
...
fn candidate_pair_sources(
    candidates: &[crate::spec::types::MappingCandidate],
    prereg_var: &str,
) -> Vec<String> {
