...
pub fn unresolved_warning(mapping: &MappingResult) -> Option<WarningItem> {
  ...
  Some(WarningItem {
    ...
    message: format!("Unable to map prereg variable '{}' to QSF column.", mapping.prereg_var),
  ...
  fn maps_condition_to_label_candidate() {
    let qsf = QsfSurveySpec {
      ...
      questions: vec![QsfQuestion {
        ...
        choices: vec![QsfChoice { value: "1".to_string(), label: "Low".to_string() }],
      }],
      embedded_data: vec![],
      embedded_data_fields: vec![QsfEmbeddedData { name: "participant_id".to_string(), default_value: None }],
